...
var replaceAsync__default = /*#__PURE__*/_interopDefaultCompat(replaceAsync);
...
function parseAttributes(element) {
    ...
    const root = nodeHtmlParser.parse(element.replace(/[\r\n]+/g, ' '));
...
function imagePreprocessor(options) {
    ...
    return {
        async markup({ content }) {
            return {
                code: await replaceAsync__default.default(content, /<Image[^>]+>/g, (element) => processImageElement(element, queue, options)),
            };
        },
    };
}
